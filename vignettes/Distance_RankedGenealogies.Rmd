...
treetypes <- c("yule","pda","ab","balanced", "unbalanced")
...
for (i in 1:length(treetypes)) {
  ...
  epsilon <- 0.001 # the same value used in Maliet, Gascuel, and Lambert (2018)
        ...
        tr.list[[i]] <- replicate(n.sim,apTreeshape::simulate_tree(epsilon=epsilon, alpha=alpha, beta=beta, N=n.tip))
        
        
        # create.Fmat is a bit slow so parallelized it when the number of simulation is large.
        Fmat.list[[i]] <-list()
        for (j  in 1:n.sim) {
            Fmat.list[[i]][[j]]<-gen_Fmat(tr.list[[i]][[j]]))
        }
}
...
# ============================================================
# Create distance matrix and do MDS  
# ============================================================
...
mds.analysis <- function(dmat, save.f.pre) {
    ...
    ## Plot medoid trees.
    png(paste(save.f.pre, 'medoid_tr.png', sep=''), width=1300, height=1000)
