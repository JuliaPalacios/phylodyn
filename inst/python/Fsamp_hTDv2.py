...
#def F_sample(oldsuff,seed):
    ...
    poolN, nodesDict, Noldsuff = createTree(oldsuff)
...
def processMutation_het_singlenode(node,name, this_vintage, F_nodes, timeVec, tMatrix,ids,idc):
    ...
    if sum([len(node.mutFromSingleton[i]) for i in list(node.mutFromSingleton.keys())])==1: ##it is a dictionary now 
        ...
        loglik += (node.mutFromSingleton[list(node.mutFromSingleton.keys())[0]][0])*np.log(timeVec[ind_marginalize][0])
    ...
    elif sum([len(node.mutFromSingleton[i]) for i in list(node.mutFromSingleton.keys())])>1: ##CHECK IF THIS ONE WORKS
...
def calcPF_het(F_nodes, family_size, oldsuff,node_group, times=None, indt=None, use_familySize = True, no_output = False):
    ...
    """
    ##  DO I STILL NEED THIS?  MAYBE IT COULD BE INSIDE. 
    #print(oldsuff)
    poolN, nodesDict, Noldsuff = createTree(oldsuff, isPF=True)
    ...
    if use_familySize:
       ...
       pF_het(oldsuff,nodesDict, F_nodes, prob, res, paths, ids,idc, timeVec=timeVec, tMatrix = tMatrix, no_output = no_output)
    
    #print("number of nodes"+str(nodesDict[0].original_node)+"and length"+str(len(nodesDict)))
    for name,node in nodesDict.items(): #CHECK!!! I DON'T KNOW WHAT THIS IS.
    ...
    #TREE LENGTHS CORRECTION TO REDO
    #print(times)
    #print(indt)
    #print(oldsuff)
    ...
    for i in b:
        ...
        if len(Ipath)==0:
            ...
            for node in parList:
                ...
                ifam_gr=np.tile(family_group[:,i],(len(parNode),1)).transpose()
                idd=np.sum(abs(node_group[:,parNode]-ifam_gr),axis=0)
